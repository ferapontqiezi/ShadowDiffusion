...
class DenoisingDiffusion(object):
    def __init__(self, args, config):
        ...
        print(f"=> total parameters in the diffusion model: {total_params}")
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if self.step % self.config.training.snapshot_freq == 0 or self.step == 1:
                    utils.logging.save_checkpoint({
                        ...
                        'params': self.args,
                        'config': self.config
                    }, filename=os.path.join(self.config.data.data_dir, 'ckpts', 'size_' + str(self.config.data.image_size) +  '_ch_' + str(self.config.model.ch) + '_epochs_' + str(self.config.training.n_epochs)))

    def sample_image(self, x_cond, x, last=True, patch_locs=None, patch_size=None):
        skip = self.config.diffusion.num_diffusion_timesteps // self.args.sampling_timesteps
        seq = range(0, self.config.diffusion.num_diffusion_timesteps, skip)
        if patch_locs is not None:
            xs = utils.sampling.generalized_steps_overlapping(x, x_cond, seq, self.model, self.betas, eta=1.0,
                                                              corners=patch_locs, p_size=patch_size)
        else:
            xs = utils.sampling.generalized_steps(x, x_cond, seq, self.model, self.betas, eta=1.0)
        if last:
            xs = xs[0][-1]
        return xs
...
class DenoisingDiffusion1(object):
    def __init__(self, args, config):
        super().__init__()
        self.args = args
        self.config = config
    ...
    def train(self, DATASET):
        ...
        if os.path.isfile(self.args.resume1):
            self.load_ddm_ckpt(self.args.resume1)
...
class DenoisingDiffusion2(object):
    def __init__(self, args, config):
        super().__init__()
        self.args = args
        self.config = config
    ...
    def train(self, DATASET):
        ...
        if os.path.isfile(self.args.resume2):
            self.load_ddm_ckpt(self.args.resume2)
