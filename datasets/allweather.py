...
class AllWeather:
    ...
    def get_loaders(self, parse_patches=True, validation='snow'):
        if validation == 'raindrop':
            print("=> evaluating ntire_24_valid_inp...")
            path = os.path.join(self.config.data.data_dir)
            filename = 'valid.txt'
            ...
            print("=> evaluating ntire_24_final_test_inp...")
            path = os.path.join(self.config.data.data_dir)
            #filename = 'valid.txt'
            filename = 'test.txt'
            # CUDA_VISIBLE_DEVICES=0 python eval_diffusion.py --config "ntire.yml" --resume '/root/autodl-tmp/ddpm/ckpts/size_64_ch_128_epochs_2000.pth.tar' --test_set 'snow' --sampling_timesteps 250 --grid_r 16
...
class AllWeatherDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        ...
        with open(train_list) as f:
            ...
            # gt_names = [re.sub(r'(\d+)_rain', r'\1_clean', i.strip().replace('input', 'gt')) for i in input_names]
    ...
    def get_images(self, index):
        ...
        if self.parse_patches:
            ...
            #print("wd_new: ", wd_new, "ht_new: ", ht_new )
...
class AllWeather1:
    ...
    def get_loaders(self, parse_patches=True, validation='snow'):
        ...
        train_dataset = AllWeatherDataset1(os.path.join(self.config.data.data_dir),
                                          n=self.config.training.patch_n,
                                          patch_size=self.config.data.image_size,
                                          transforms=self.transforms,
                                          filelist='train.txt',
                                          parse_patches=parse_patches)
        val_dataset = AllWeatherDataset1(path, n=self.config.training.patch_n,
                                        patch_size=self.config.data.image_size,
                                        transforms=self.transforms,
                                        filelist=filename,
                                        parse_patches=parse_patches)
...
class AllWeatherDataset1(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        super().__init__()
...
class AllWeather2:
    ...
    def get_loaders(self, parse_patches=True, validation='snow'):
        ...
        train_dataset = AllWeatherDataset2(os.path.join(self.config.data.data_dir),
                                          n=self.config.training.patch_n,
                                          patch_size=self.config.data.image_size,
                                          transforms=self.transforms,
                                          filelist='train.txt',
                                          parse_patches=parse_patches)
        val_dataset = AllWeatherDataset2(path, n=self.config.training.patch_n,
                                        patch_size=self.config.data.image_size,
                                        transforms=self.transforms,
                                        filelist=filename,
                                        parse_patches=parse_patches)
...
class AllWeatherDataset2(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        super().__init__()
