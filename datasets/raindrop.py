...
class RainDrop:
    ...
    def get_loaders(self, parse_patches=True, validation='raindrop'):
        ...
        val_dataset = RainDropDataset(dir=os.path.join(self.config.data.data_dir, 'data', 'raindrop', 'test_b'),
                                      n=self.config.training.patch_n,
                                      patch_size=self.config.data.image_size,
                                      transforms=self.transforms,
                                      filelist=None,
...
class RainDropDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True):
        ...
        if filelist is None:
            ...
            # print("data_path", os.path.join(raindrop_dir, 'data'))
            raindrop_inputs = os.path.join(raindrop_dir, 'data')
